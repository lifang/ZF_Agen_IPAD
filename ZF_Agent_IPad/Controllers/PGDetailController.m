...
- (void)initSubViewWithAllTerminals:(BOOL)isAll {
    ...
    CGFloat rightSpace = 60.f;
    CGFloat labelHeight = 30.f;
    CGFloat lineSpace = 2.f;
    CGFloat middleSpace = 20.f;
    CGFloat rightLabelWidth = 100.f; //右边label宽度
    //配货对象
    UILabel *preprareLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, labelHeight)];
    [self setLabel:preprareLabel withContent:@"配货对象"];
    //划线
    originY += labelHeight + lineSpace;
    UIView *firstLine = [[UIView alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, 1)];
    firstLine.backgroundColor = kMainColor;
    [_scrollView addSubview:firstLine];
    
    //代理商
    originY += 1 + lineSpace;
    UILabel *agentLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace - rightLabelWidth, labelHeight)];
    [self setLabel:agentLabel withContent:_agentName];
    //总计
    UILabel *totalLabel = [[UILabel alloc] initWithFrame:CGRectMake(wide - rightSpace - rightLabelWidth, originY, rightLabelWidth, labelHeight)];
    [self setLabel:totalLabel withContent:[NSString stringWithFormat:@"总计%d件",_count]];
    totalLabel.textAlignment = NSTextAlignmentRight;
    
    //配货日期
    originY += labelHeight + middleSpace;
    UILabel *timeTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, labelHeight)];
    [self setLabel:timeTitleLabel withContent:@"配货日期"];
    //划线
    originY += labelHeight + lineSpace;
    UIView *secondLine = [[UIView alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, 1)];
    secondLine.backgroundColor = kMainColor;
    [_scrollView addSubview:secondLine];
    //时间
    originY += 1 + lineSpace;
    UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, labelHeight)];
    [self setLabel:timeLabel withContent:_createTime];
    
    //操作人标题
    originY += labelHeight + middleSpace;
    UILabel *operationTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, labelHeight)];
    [self setLabel:operationTitleLabel withContent:@"操作人"];
    //划线
    originY += labelHeight + lineSpace;
    UIView *thirdLine = [[UIView alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, 1)];
    thirdLine.backgroundColor = kMainColor;
    [_scrollView addSubview:thirdLine];
    //操作人
    originY += 1 + lineSpace;
    UILabel *operationLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, labelHeight)];
    [self setLabel:operationLabel withContent:_opreationer];
    
    //终端号标题
    originY += labelHeight + middleSpace;
    UILabel *terminalTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace - rightLabelWidth, labelHeight)];
    [self setLabel:terminalTitleLabel withContent:@"终端号"];
    
    //总计
    NSArray *terminalList = [_terminalString componentsSeparatedByString:@","];
    UILabel *terminalCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(wide - rightSpace - rightLabelWidth, originY, rightLabelWidth, labelHeight)];
    [self setLabel:terminalCountLabel withContent:[NSString stringWithFormat:@"总计%ld件",(unsigned long)[terminalList count]]];
    terminalCountLabel.textAlignment = NSTextAlignmentRight;
    //划线
    originY += labelHeight + lineSpace;
    UIView *forthLine = [[UIView alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, 1)];
    ...
    if (isAll) {
        for (int i = 0; i < [terminalList count]; i++) {
            UILabel *terminalLabel = [[UILabel alloc] initWithFrame:CGRectMake(leftSpace, originY, wide - leftSpace - rightSpace, labelHeight)];
    ...
    _scrollView.contentSize = CGSizeMake(wide, originY);
}
